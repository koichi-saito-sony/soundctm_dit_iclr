...
from sa_edm.unet.unet_1d_blocks import (
...
class CTMUNet1DConditionModel(ModelMixin, ConfigMixin):
    r"""
    ...
    def __init__(
        ...
        block_out_channels: Tuple[int] = (320, 320, 1280, 1280),
        ...
        self.s_time_embedding = TimestepEmbedding(
            in_channels=timestep_input_dim,
            time_embed_dim=time_embed_dim,
            act_fn=act_fn,
            post_act_fn=timestep_post_act,
        ...
        if mid_block_type == "UNetMidBlock1DCrossAttn":
            self.mid_block = UNetMidBlock1DCrossAttn(
    ...
    def _set_time_proj(
            ...
            if time_embedding_type == "fourier":
                ...
                self.s_time_proj = GaussianFourierProjection(
                    time_embed_dim // 2, set_W_to_weight=False, log=False, flip_sin_to_cos=flip_sin_to_cos
                )
                ...
                self.s_time_proj = Timesteps(block_out_channels[0], flip_sin_to_cos, freq_shift)
    ...
    def forward(
        ...
        s_timestep: Optional[Union[torch.Tensor, float, int]]= None,
        embedd_cfg: Optional[torch.Tensor]= None,
        ...
        timestep_embed = self.time_proj(timesteps).to(dtype=sample.dtype)
        timestep_embed = self.time_embedding(timestep_embed)
        aug_emb = None
        emb = timestep_embed + aug_emb if aug_emb is not None else timestep_embed
        
        # NOTE: define how to embed 's' of CTM here.
        # if s_timestep is not None:
        if not torch.is_tensor(s_timestep):
            raise NotImplementedError
            # TODO: this requires sync between CPU and GPU. So try to pass timesteps as tensors if you can
            # This would be a good case for the `match` statement (Python 3.10+)
        elif len(s_timestep.shape) == 0:
            s_timestep = s_timestep[None].to(sample.device)

        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        s_timestep = s_timestep.expand(sample.shape[0])

        s_emb = self.s_time_proj(s_timestep).to(dtype=sample.dtype)
        embedd_s = self.s_time_embedding(s_emb)
        
        emb = emb + embedd_s
        
        # if embedd_cfg is not None:
        emb  = emb + embedd_cfg
